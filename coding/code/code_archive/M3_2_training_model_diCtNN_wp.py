...
variables =	{
    "global" : {
        ...
        "grid_search_name" : 'diCtNN_Model_26_90epochs_wp', #pro run bitte anpassen
    ...
    for run_number, current_params in enumerate(all_params):
        ...
        if run_number not in [26]:
            ...
            if variables['global']['dimension_of_model'] == '1D': 
                ...
                train_vectors = torch.load(variables['global']['path'] + "train_vectorized_1d_wp.pt")
                train_labels = torch.load(variables['global']['path'] + "train_labels_1d_wp.pt")

                # val set
                val_vectors = torch.load(variables['global']['path'] + "val_vectorized_1d_wp.pt")
                val_labels = torch.load(variables['global']['path'] + "val_labels_1d_wp.pt")

            elif variables['global']['dimension_of_model'] == '2D': 
                # loading the data saved during preprocessing: 
                train_vectors = torch.load(variables['global']['path'] + "train_vectorized_2d_wp.pt")
                train_labels = torch.load(variables['global']['path'] + "train_labels_2d_wp.pt")

                # val set
                val_vectors = torch.load(variables['global']['path'] + "val_vectorized_2d_wp.pt")
                val_labels = torch.load(variables['global']['path'] + "val_labels_2d_wp.pt")
