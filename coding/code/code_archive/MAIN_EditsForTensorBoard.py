...
import pandas as pd
from M2_0_NNSetup_EditsForTensorBoard import *
from M2_1_CNN_1d import CNN_1d
from M2_1_CNN_1d_experiment_EditsForTensorBoard import CNN_1d_experiment
import json
from torch.utils.tensorboard import SummaryWriter # to write information to tensorboard
# --> needs pip install tensorboard==1.14.0 to work
...
# Setup the writer for Tensor Board to Track Training
path_writer = path + uniqueOutputPrefix + 'TensorBoardData'
...
# getting sample data for TensorBoard
dataiter = iter(setup.dataset_loader)
tweetBertTensor, labels = dataiter.next()
tweetBertTensor = tweetBertTensor.unsqueeze(2)

# writing model and sample data to Tensorboard
writer.add_graph(model, tweetBertTensor)
writer.close()
...
# define Optimizer
setup.setOptimizer()
# TODO: Do we need to set a momentum here? It's supposed to help the model get over potential local minima and converge better.
